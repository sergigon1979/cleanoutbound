...
In 2002, a Lisp programmer wrote an essay about Bayesian filters. It sparked the first arms race between human attackers and AI defenses. The defenders won, but the conditions enabling that victory no longer exist.
</span>
...
The screenshot above is a SpamAssassin report from 2003. The score reads 5.3, just above the threshold, with every rule that fired listed alongside its point value and its contribution to the total. The filter told the attacker exactly which rules had triggered, how much each one cost, and how close the message had come to passing. A spammer could compose a message, run it through SpamAssassin, read the score, and adjust until it slipped under the threshold. Rewrite the subject line. Drop a trigger word. Add a line of innocuous text. Check again. 4.9. Send. Defense as open-book exam.
...
Brightmail, a commercial anti-spam service used by major Internet Service Providers, took a different approach. It deployed a global network of decoy email addresses — honeypots. When a message arrived at a honeypot, Brightmail computed a digital fingerprint of it and distributed that fingerprint to subscribers. If an email matched a known fingerprint, it was spam. The system was elegant against true bulk: identical messages sent to millions of addresses all produced the same fingerprint. But it was blind to variation. Change a single character, a comma or a space or a digit in a phone number, and the fingerprint changed entirely. The message sailed through.⁷

All three failed differently, but they shared one fatally flawed assumption: that spam was a fixed target. Certain words, certain patterns. Of course, it wasn't. It was changed frequently and easily. Even more so when the filter was making obvious — public even, as in the case of SpamAssassin — what those certain words and certain patterns were.

<p style="margin-top: 1.5rem;"><strong>A Plan for Spam</strong></p>

In August 2002, Paul Graham published an essay called "A Plan for Spam" on his personal website. Graham was a programmer and entrepreneur who had sold his startup Viaweb to Yahoo in 1998. He had since been designing a new programming language called Arc, and building a web-based email client to test it. The system needed a spam filter. 

Graham's first approach was rather like SpamAssassin's: writing rules by hand. He grew sick of it. "To recognize individual spam features you have to try to get into the mind of the spammer," he wrote, "and frankly I want to spend as little time inside the minds of spammers as possible." He tried a statistical approach instead, and found immediately that it was much cleverer than he had been. The essay was a report on that experiment.⁸

Instead of writing rules about what spam says, count tokens and learn what spam looks like statistically. The filter would be trained, not programmed.
...
A token, in Graham's system, was not a word as a dictionary defines it, but any character string the parser encountered: a conventional word like "meeting," a cluster of exclamation marks (!!!), an HTML fragment like `<font color="red">`, a misspelling, a string of digits. The filter only cared about frequency, not meaning.¹⁰

Before it could classify anything, the filter had to be trained. The user fed it two collections: a folder of known spam and a folder of known legitimate messages, which the community began calling "ham." The filter walked through spam and ham and counted how often each token appeared in each. After training, every token was assigned a number between 0 and 1 representing how likely a message containing it was to be spam.

"V1agra" was not a clever disguise anymore. It was just a token, linked to spam just as frequently as its more modest relative "Viagra." Every new mutation was simply a new token to be counted, classified, and filed. And the filter learned the other side just as well. "Invoice," "Tuesday," "deadline" — tokens saturated in your daily work scored near 0.0. For the first time, the absence of normal language was itself a signal.

My probability table was different from yours. "Explosive growth" was standard Series B pitch language to a venture capitalist, but penis enlargement pill spam to an engineer. The words you used every day formed a shield around your inbox. Every previous filter was a detector of bad signals. Graham's was equally a detector of good. A message saturated in the vocabulary of your daily work was not merely unpenalized. It was actively protected.

In traditional programming, a human writes the logic and the machine executes it. In machine learning, a human provides the data. The resulting model, the thousands of weights and probabilities that determine filter behavior, is produced mechanically by the process. Not even the programmer could look at a trained filter and predict with certainty what it would do with a given message. You could inspect it, not read it.

Graham built this in a few hundred lines of Lisp, running on a single laptop. But the structure was there: training data, probabilistic classification, a verdict that emerged from patterns rather than rules. The first AI built to defend human attention was a spam filter running on someone's IBM ThinkPad.¹²

<p style="margin-top: 1.5rem;"><strong>Probing the Black Box</strong></p>

The industry moved fast. Within eighteen months, Graham's approach had been absorbed into infrastructure at every scale. SpamAssassin integrated a Bayesian engine alongside its hand-written rules; sysadmins could now train a local probability map on their own servers.¹³ Mozilla Thunderbird placed machine learning at the center of the consumer experience, its "Junk" button quietly reframing the act of deleting spam as an act of training.¹⁴ Microsoft's SmartScreen aggregated classifications from hundreds of thousands of volunteers to train a single shared model for all of Outlook and Exchange.¹⁵ Each step concentrated more labeling power and more compute in fewer hands.
...
Spammers understood the logic instantly: if ham tokens protect a message, inject ham tokens. They began appending blocks of innocent text to their emails, paragraphs lifted from novels, random dictionary words, fragments of news articles. Some used passages from Moby Dick.¹⁶ The filter would encounter "whale" and "harpoon" sitting beneath an advertisement for discount Viagra. These strange messages attracted the attention of experimental literature aficionados, yet they rarely worked as an evasion strategy.

When the email arrived, the filter collected the fifteen most "interesting" tokens in the message, the ones whose spam probability lay farthest from 0.5 in either direction, and combined them. "Viagra" (near 1.0) and "pharmacy" (near 1.0) pulled toward spam. For the poisoning to work, the injected tokens needed to pull just as hard toward ham. They needed to score near 0.0.

By limiting the calculation to the most extreme outliers, Graham forced the attacker to produce tokens the filter had a strong opinion about. Melville's prose rarely qualified. Words the filter had never seen landed at a neutral 0.4, too close to the center to even enter the calculation. Only tokens near 0.0, the kind reserved for familiar language in legitimate inboxes, could cancel out a "Viagra." The technique received a scary name, Bayesian poisoning, but it was crude and ineffective.
...
The more substantive attack was adversarial probing. The filter was a mathematical function. Understanding it was unnecessary. Mapping it was enough. Professional spammers set up test labs with private installations of open-source filters and ran canary messages through them before launching a campaign. If the local filter caught the message, a script would swap synonyms: “money” for “cash,” “opportunity” for “proposition,” until the score dropped below the threshold. The smarter operators skipped the test lab and ran the experiment live, sending a hundred variants to real inboxes and adjusting until the filter’s shape revealed itself. Machine-learning researchers now call this a substitution attack. In 2003, it was just called getting the email through.

Step back and watch the picture: every structural advantage favored the attacker. Sending was free; failure carried no cost. The model was a fixed mathematical function; it could be mapped. The defender couldn't afford aggressive filtering; a false positive meant a missed invoice, a lost job offer, a deal that never happened. The blame for a false positive always fell on the filter, never on the spammer who caused the noise in the first place.
...
Within months of Bayesian filters becoming standard, spammers rendered their pitches as images, turning text the filter could score into pixels it could not. When filters learned to read images, the attackers moved again, first to botnets and then to distributed sending across thousands of IPs and domains. The inbox had stopped asking what a message said and begun asking who was sending it. Each defensive victory addressed one tactic; each time, the attacker walked to the next undefended front.

But those were battles fought later, on other fronts.

The Bayesian victory over text spam was momentous. It held. The text front was won and would remain so for twenty years.¹⁹ When it finally reopened, sometime around 2023,²⁰ it wasn't because the attackers had found a way to beat the filter. The conditions that made the filter work had simply disappeared.
...
Then there was compute. No other entity could run probabilistic classification across billions of messages in real time, retrain continuously, and maintain personalized filters for hundreds of millions of users. The defender had a resource advantage the attacker couldn't match. Today, attackers run open-source language models on a single consumer GPU, testing thousands of variations against simulated filters before a single message is sent. The capabilities are now matched. The constraints are not: the defender still can't afford a false positive.
    ...
    <div class="charts-row">
      ...
      <div class="chart-cell">
        ...
        <svg class="chart-svg" viewBox="0 0 280 164" preserveAspectRatio="xMidYMid meet">
          ...
          <!-- Ham curve - fill then stroke -->
          <path class="ham-fill" d="M 36.0,133.3 L 40.5,132.2 L 45.0,130.8 L 49.4,129.0 L 53.9,126.6 L 58.4,123.7 L 62.9,120.0 L 67.4,115.7 L 71.8,110.5 L 76.3,104.5 L 80.8,97.7 L 85.3,90.2 L 89.8,82.0 L 94.2,73.3 L 98.7,64.4 L 103.2,55.5 L 107.7,46.9 L 112.2,38.9 L 116.6,31.9 L 121.1,26.0 L 125.6,21.6 L 130.1,18.9 L 134.6,18.0 L 139.0,18.9 L 143.5,21.6 L 148.0,26.0 L 152.5,31.9 L 157.0,38.9 L 161.4,46.9 L 165.9,55.5 L 170.4,64.4 L 174.9,73.3 L 179.4,82.0 L 183.8,90.2 L 188.3,97.7 L 192.8,104.5 L 197.3,110.5 L 201.8,115.7 L 206.2,120.0 L 210.7,123.7 L 215.2,126.6 L 219.7,129.0 L 224.2,130.8"/>
...
<div class="co-exhibit co-exhibit--takeaways">
  ...
  <div class="box">
    ...
    <ul>
      <li><strong>A Plan for Spam sparked the first AI arms race.</strong> Before anyone used the term, spam filters were doing machine learning: training on labeled data, making probabilistic predictions, fighting adversarial attacks at industrial scale. The vocabulary caught up twenty years later.</li>
...
<p>¹² Graham does not specify which machine he used in 2002. In "Return of the Mac" (March 2005), he describes switching to a Powerbook at the end of 2004 after his IBM ThinkPad's hard disk died. The ThinkPad is a reasonable inference.
